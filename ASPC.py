...
class ASPC(object):
    ...
    def sparse_loss(self,y_true,y_pred):
        alpha = 7.5e-5 # 预设权重
        p = 0.2 # 稀疏度
        hidden_out = self.autoencoder.get_layer('encoder_3').output
        MSE = tf.reduce_sum(tf.squared_difference(y_true,y_pred))
        pj = tf.reduce_mean(hidden_out,axis=0)+tf.ones_like(tf.reduce_mean(hidden_out,axis=0))*1e-8
        kl_loss = alpha*tf.reduce_sum(p * tf.log(p / pj) + (1 - p) * tf.log((1 - p) / (1 - pj)))
        total_loss = MSE + kl_loss
        return total_loss

    def fit(self, x, y=None, batch_size=256, epochs=100,
            ae_weights=None, save_dir='result/temp', tol=0.001,
            use_sp=True, da_s2=False):
